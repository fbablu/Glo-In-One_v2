...
class MOTSValDataSet_300um(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(256, 256), mean=(128, 128, 128), scale=False,
                 mirror=False, ignore_label=255, edge_weight=1):
        self.root = root
        self.list_path = list_path
    ...
    def __getitem__(self, index):
        datafiles = self.df.iloc[index]
        # read png file
        origin_image = plt.imread(datafiles["image_path"])
        #label = plt.imread(datafiles["label_path"])
        ...
        # data augmentation
        image = origin_image[:, :, :3]
        #label = label[:, :, :3]
        origin_shape = image.shape[0]

        if (image.shape[0] <= 1400) & (image.shape[0] >= 1000): # X40 about 1200 * 1200 pixels
            #backup_shape = image.shape[0]
            image = resize(image, [int(image.shape[0] / 2), int(image.shape[1] / 2), 3])
            image = image[int(image.shape[0]/2) - 256:int(image.shape[1]/2) + 256, int(image.shape[0]/2) - 256:int(image.shape[1]/2) + 256, :]


        if (image.shape[0] <= 680) & (image.shape[0] >= 580):  # X20 about 600 * 600 pixels
            #backup_shape = image.shape[0]
            #image = resize(image, [int(image.shape[0] / 2), int(image.shape[1] / 2), 3])
            image = image[int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, :]

        if (image.shape[0] <= 480) & (image.shape[0] >= 320):  # X20 about 600 * 600 pixels
            #backup_shape = image.shape[0]
            image = resize(image, [int(image.shape[0] * 2), int(image.shape[1] * 2), 3])
            image = image[int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, :]

        if (image.shape[0] == 2048) & (image.shape[0] == 2048):  # X20 about 600 * 600 pixels
            #backup_shape = image.shape[0]
            image = resize(image, [1024, 1024, 3])
            image = image[int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, :]

        # if (image.shape[0] <= 400) & (image.shape[0] >= 380):  # X20 about 600 * 600 pixels
        #     #backup_shape = image.shape[0]
        #     image= cv2.copyMakeBorder(
        #         image,
        #         int((int(image.shape[0] * 2) - 512) / 2),
        #         int((int(image.shape[0] * 2) - 512) / 2),
        #         int((int(image.shape[0] * 2) - 512) / 2),
        #         int((int(image.shape[0] * 2) - 512) / 2), cv2.BORDER_CONSTANT, value=0)
        #
        #     image = image[int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, int(image.shape[0] / 2) - 256:int(image.shape[1] / 2) + 256, :]
        ...
        #origin_image = origin_image[0].transpose((2, 0, 1))
        origin_image = origin_image.astype(np.float32)
        name = datafiles["image_path"]

        return image.copy(), name, task_id, scale_id,  origin_shape, origin_image.copy()

class MOTSValDataSet_normal(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(256, 256), mean=(128, 128, 128), scale=False,
                 mirror=False, ignore_label=255, edge_weight=1):
        self.root = root
        self.list_path = list_path
