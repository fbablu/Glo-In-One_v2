    ...
    with Engine(custom_parser=parser) as engine:
        ...
        with torch.no_grad():
            for iter, batch in enumerate(valloader):
                ...
                output_folder = os.path.join(args.output_folder, 'testing_%s' % (args.validsetname))
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    preds_0, _ = model(images, torch.ones(batch_size).cuda() * 0, scales)
                    preds_1, _ = model(images, torch.ones(batch_size).cuda() * 1, scales)
                    preds_2, _ = model(images, torch.ones(batch_size).cuda() * 2, scales)
                    preds_3, _ = model(images, torch.ones(batch_size).cuda() * 3, scales)
                    preds_4, _ = model(images, torch.ones(batch_size).cuda() * 4, scales)
                    preds_5, _ = model(images, torch.ones(batch_size).cuda() * 5, scales)
                    preds_6, _ = model(images, torch.ones(batch_size).cuda() * 6, scales)
                    preds_7, _ = model(images, torch.ones(batch_size).cuda() * 7, scales)
                    preds_8, _ = model(images, torch.ones(batch_size).cuda() * 8, scales)
                    preds_9, _ = model(images, torch.ones(batch_size).cuda() * 9, scales)
                    preds_10, _ = model(images, torch.ones(batch_size).cuda() * 10, scales)
                    preds_11, _ = model(images, torch.ones(batch_size).cuda() * 11, scales)
                    preds_12, _ = model(images, torch.ones(batch_size).cuda() * 12, scales)
                    preds_13, _ = model(images, torch.ones(batch_size).cuda() * 13, scales)
                    preds_14, _ = model(images, torch.ones(batch_size).cuda() * 14, scales)
                    ...
                    for pi in range(len(images)):
                        ...
                        merged_mask = np.concatenate(
                            (np.expand_dims(back_mask_0, axis=2), np.expand_dims(back_mask_1, axis=2),
                             ...
                             np.expand_dims(back_mask_14, axis=2)),
                            axis=2)
                        np.save(os.path.join(output_folder, os.path.basename(filename[pi]) + '_preds_merged.npy'), merged_mask)
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 7, scales)
            #     now_task = torch.tensor(7)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 8, scales)
            #     now_task = torch.tensor(8)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 9, scales)
            #     now_task = torch.tensor(9)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 10, scales)
            #     now_task = torch.tensor(10)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 11, scales)
            #     now_task = torch.tensor(11)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 12, scales)
            #     now_task = torch.tensor(12)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 13, scales)
            #     now_task = torch.tensor(13)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
            ...
            #
            #     preds, _ = model(images, torch.ones(left_size).cuda() * 14, scales)
            #     now_task = torch.tensor(14)
            #     # print("PREDS.shape:", preds.shape)
            #
            #     # now_preds = preds[:, 1, ...] > preds[:, 0, ...]
